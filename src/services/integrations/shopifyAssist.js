...
const getCreds = async (workspace) => {
  const storeUrl = workspace.integrations?.shopify?.storeUrl;
  if (!storeUrl) return null;

  let token = workspace.integrations?.shopify?.accessToken;
  if (!token) {
  ...
  try {
    return { storeUrl, token: decrypt(token) };
  } catch {
    return null;
  }
};
...
/**
 * @returns {Promise<string|null>} context block to inject, or null.
 */
